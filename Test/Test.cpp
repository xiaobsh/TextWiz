...
int main()
{
	try
	{
		...
		testFile.AddAtLine(137, testFile.GetByLine(71));
		testFile.save();
		...
		for (TextWiz::TextWiz_Position i : VectorToSaveResults)
		{
			...
			testFile.DelLine(i.line); // Not to delete all lines that contain 'software'. Line number will be changed each time you delete a line. So it's suggested to delete lines from the biggest line number to reach your purpose.
		}
		testFile.exit();
		logger.open("ORIGIN");
		...
		if (testFile != Expectation) throw 2;
		testFile.close();
	}
